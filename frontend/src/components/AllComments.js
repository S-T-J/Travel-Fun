	...
	useEffect(async () => {
		...
		function findReplies(commentId, obj) {
			let replies = res.data
				.reverse()
				.filter((eachComment) => eachComment.commentId == commentId);
			obj["replies"] = replies;
	...
	const ShowReplies = () => {
		...
		function digDeeper(num, replies) {
			...
			for (let each of replies) {
				all.push(
					<>
						<div style={{ marginLeft: num * 50 + "px", marginTop: num * 0 + "px" }}>
							{each.text}

							<EachReply eachComment={each} key={each._id} {...props} />


							{num % 2 === 0 ? <vr style={{ backgroundColor: 'white' }}></vr> : <vr style={{ backgroundColor: '#eceeef' }}></vr>}
